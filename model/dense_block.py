...
class DenseBlock(tf.keras.Model):
    def __init__(self, num_layers, growth_rate=32):
        ...
        for _ in range(num_layers):
            ...
            self.dropout1_layers.append(tf.keras.layers.Dropout(rate=0.4))
            ...
            self.dropout2_layers.append(tf.keras.layers.Dropout(rate=0.4))
    ...
    def call(self, inputs, training=False):
        ...
        for i in range(self.num_layers):
            ...
            x = self.bn1_layers[i](x, training=training)
            x = tf.keras.layers.Activation('relu')(x)
            x = self.conv1_layers[i](x)
            x = self.dropout1_layers[i](x, training=training)

            x = self.bn2_layers[i](x, training=training)
            x = tf.keras.layers.Activation('relu')(x)
            x = self.conv2_layers[i](x)
            x = self.dropout2_layers[i](x, training=training)
